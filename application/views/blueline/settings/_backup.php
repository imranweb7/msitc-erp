<?php   
$attributes = array('class' => '', 'id' => 'backup_form');
echo form_open_multipart($form_action, $attributes); 
?>
<div class="alert alert-warning"><?php echo $this->lang->line('application_restore_notice');?></div><br/>
     
<div class="form-group">
                <label for="userfile"><?php echo $this->lang->line('application_backup_file_zip');?></label><div>
                <input id="uploadFile" class="form-control uploadFile" placeholder="Choose File" disabled="disabled" />
                          <div class="fileUpload btn btn-primary">
                              <span><i class="fa fa-upload"></i><span class="hidden-xs"> <?php echo $this->lang->line('application_select');?></span></span>
                              <input id="uploadBtn" type="file" name="userfile" class="upload" />
                          </div>
                  </div>
              </div>       

        <div class="modal-footer">
        <input type="submit" name="send" class="btn btn-primary" value="<?php echo $this->lang->line('application_restore_backup');?>"/>
        <a class="btn" data-dismiss="modal"><?php echo $this->lang->line('application_close');?></a>
        </div>

<?php echo form_close(); ?>